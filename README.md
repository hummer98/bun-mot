...
[mado](https://github.com/yujiyamamoto/mado) (本リポジトリと同じ作者の Markdown プレビューア)
を bun-mot で E2E テストするための **手順とスモークスクリプト**を同梱している。

- 手順: [`docs/mado-integration.md`](docs/mado-integration.md) — mado 側で必要な最小 diff (bridge 起動コード + `--env='BUN_MOT_*'`)
- スモーク実行: `MADO_DIR=/path/to/mado bun run mado:smoke`

> **macOS GUI 環境必須**: `bun run mado:smoke` を実行すると mado のウィンドウが立ち上がる。
> SSH / ヘッドレス環境では実行不可。CI には組み込まない (環境依存が大きいため、手動スモークに留める)。
> `MADO_DIR` 未指定時は使い方を表示して exit 1 する (作者個人 path をデフォルトにしない)。
...
MADO_DIR=/path/to/mado bun run mado:smoke   # mado に対する手動スモーク (macOS GUI 必須)
```
...
- [`docs/mado-integration.md`](docs/mado-integration.md) は mado 側 PR で必要な最小 diff の参考実装。
